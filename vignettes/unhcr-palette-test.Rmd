...
```{r}
g_points + scale_color_discrete_unhcr(palette = "test_pal")
g_distrb +  scale_fill_discrete_unhcr(palette = "test_pal")
```
...
```{r}
g_points + 
  scale_color_discrete_unhcr(palette = "test_pal", extend = TRUE)

g_distrb + 
  scale_fill_discrete_unhcr(palette = "test_pal", extend = TRUE)
```
...
ggplot(points, aes(x, y, col = assessment)) + 
  geom_point() + 
  scale_color_opinionated_unhcr(palette = "test_pal")
```
...
```{r}
g_points_cont + 
  scale_color_diverging_unhcr(palette = "test_pal", midpoint = 5)
```
...
```{r}
g_points_cont + 
  scale_color_continuous_unhcr(palette = "test_pal")
```
